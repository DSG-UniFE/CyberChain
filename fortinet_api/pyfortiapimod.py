...
class FortiGate:
    def __init__(self, ipaddr, username, password, timeout=10, vdom="root", port="443", protocol="http"):
        self.ipaddr = ipaddr
        ...
        self.urlbase = "{protocol}://{ipaddr}:{port}/".format(protocol=self.protocol, ipaddr=self.ipaddr,
                                                              port=self.port)
        self.session = None
    ...
    def does_exist(self, object_url):
        ...
        """
        request = self.session.get(object_url, verify=False, timeout=self.timeout, params='vdom=' + self.vdom)
    ...
    def get(self, url):
        ...
        """
        request = self.session.get(url, verify=False, timeout=self.timeout, params='vdom=' + self.vdom)
        if request.status_code == 200:
    ...
    def put(self, url, data, params):
        ...
        result = self.session.put(url, data=data, verify=False, timeout=self.timeout,
    ...
    def post(self, url, data):
        ...
        """
        result = self.session.post(url, data=data, verify=False, timeout=self.timeout,
                                   params='vdom=' + self.vdom)
    ...
    def delete(self, url):
        ...
        """
        result = self.session.delete(url, verify=False, timeout=self.timeout, params='vdom=' + self.vdom).status_code
        return result
    ...
    def update_firewall_address(self, address, data):
        ...
        if self.session is None:
            ...
            if not self.does_exist(api_url):
                logging.error(
                    f'Requested address "{address}" does not exist in Firewall config.'.format(address=address))
                return 404
    ...
    def create_firewall_address(self, address, data):
        ...
        if self.session is None:
            ...
            if self.does_exist(api_url + address):
                return "424 - parameters are missing or parameters are incorrectly handled"
    ...
    def move_rule(self, policy_id, where, relative_policy_id):
        # example: http://192.168.51.2/api/v2/cmdb/firewall/policy/1?action=move&before=66

        """
        Move Rule position

        :param policy_id: Policy id of the rule to move
        :param where: after or before
        :param relative_policy_id: Policy id of the reference rule
        ...
        if self.session is None:
            ...
            api_url = self.urlbase + f"api/v2/cmdb/firewall/policy/{policy_id}"
            # print(api_url)
            parameters = dict(action='move')
            parameters[where] = relative_policy_id
            result = self.put(url=api_url, data="", params=parameters)
